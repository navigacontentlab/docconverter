...
func TestNewsItemToDoc_Regressions(t *testing.T) {
	...
	for _, i := range files {
		...
		t.Run(i.Name(), func(t *testing.T) {
			...
			_, _ = docconverter.NewsItemToDoc(item, &opts)
		})
	}
}
...
func checkNamespaces(t *testing.T, testFile string, xmlDocBytes []byte) {
	...
	if xmlns == nil {
		t.Errorf("%s", fmt.Sprintf("generated root is missing xmlns: %s", testFile))
	} else if strings.Compare(xmlns.Value, "http://iptc.org/std/nar/2006-10-01/") != 0 {
		t.Errorf("%s", fmt.Sprintf("generated root does not have expected xmlns: %s", testFile))
	}
...
func TestConvertingNewsmlRoundtrip(t *testing.T) {
	...
	testData, err = os.ReadFile(configWithMetaFile)
	must(t, err, "could not open file")
...
func TestCheckSchemaInIDFLinks(t *testing.T) {
	testData, err := os.ReadFile("testdata/text.xml")
	must(t, err, "could not open file")

	var original newsml.NewsItem
	err = internal.UnmarshalXML(testData, &original)
	must(t, err, "could not unmarshal file")

	customOptions := newsml.Options{}
	o := newsml.DefaultOptions()

	docconverter.MergeOptions(&o, &customOptions)

	navigadoc, err := docconverter.NewsItemToDoc(&original, &o)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInvalidDatesShouldNotBeDefaulted(t *testing.T) {
	...
	for _, tc := range cases {
		testXML, err := os.ReadFile(tc.file)
		...
		opts := newsml.DefaultOptions()
		_, err = docconverter.NewsItemToDoc(&original, &opts)
...
func TestDefaultConfigOverride(t *testing.T) {
	testData, err := os.ReadFile("./testdata/custom-config-overrides.json")
	must(t, err, "could not open file")
	customConfig := string(testData)
	...
	for i := range tests {
		...
		t.Run(test.xml, func(t *testing.T) {
			...
			navigadoc, err := docconverter.XMLToDoc(string(testXML), opts)
			if test.expectError && err != nil {
				return
			}
			must(t, err, "failed ItemToDoc")

			xmlDoc, err := docconverter.DocToItem(navigadoc, opts)
			must(t, err, "failed DocToItem")
...
func newsitemConversionRoundtrip(t *testing.T, test TestData, opts *newsml.Options) {
	...
	generatedXML, err := docconverter.DocToNewsItem(navigadoc, opts)
	must(t, err, "failed DocToNewsItem")
...
func conceptConversionRoundtrip(t *testing.T, test TestData, opts *newsml.Options) {
	want, err := os.ReadFile(test.xml)
	must(t, err, "could not open file")

	original := newsml.ConceptItem{}

	// Read the xml into the ConceptItem
	err = internal.UnmarshalXML(want, &original)
	must(t, err, "could not unmarshal file")

	if test.customConfig != "" {
		o := newsml.DefaultOptions()
		opts = &o
		testData, err := os.ReadFile(test.customConfig)
		must(t, err, "could not open file")
		cfg := &newsml.Options{}
		...
		docconverter.MergeOptions(opts, cfg)
	}

	// Convert the ConceptItem to NavigaDoc format
	document, err := docconverter.ConceptItemToDoc(&original, opts)
	if test.expectError && err != nil {
		return
	}
	must(t, err, "failed ConceptItemToDoc")

	// Concert the NavigaDoc back to a ConceptItem
	got, err := docconverter.DocToConceptItem(document, opts)
	must(t, err, "failed DocToConceptItem")
...
func TestGeneratedDocument(t *testing.T) {
	...
	testData, err := os.ReadFile(test.xml)
	must(t, err, "could not open file")
	...
	opts := newsml.DefaultOptions()
	document, err := docconverter.ItemToDoc(&original, &opts)
	must(t, err, "failed ItemToDoc")
	...
	got, err := docconverter.DocToItem(&newDoc, &opts)
	must(t, err, "failed DocToItem")
...
func listConversionRoundtrip(t *testing.T, test TestData, opts *newsml.Options) {
	...
	document, err := docconverter.ListToDoc(&original, opts)
	must(t, err, "failed ListToDoc")

	// Concert the NavigaDoc back to a List
	got, err := docconverter.DocToList(document, opts)
	must(t, err, "failed DocToList")
...
func packageConversionRoundtrip(t *testing.T, test TestData, opts *newsml.Options) {
	...
	document, err := docconverter.PackageToDoc(&original, opts)
	must(t, err, "failed PackageToDoc")

	// Concert the NavigaDoc back to a Package
	got, err := docconverter.DocToPackage(document, opts)
	must(t, err, "failed DocToPackage")
...
func assignmentConversionRoundtrip(t *testing.T, test TestData, opts *newsml.Options) {
	...
	document, err := docconverter.AssignmentToDoc(&original, opts)
	if test.expectError && err != nil {
		return
	}
	must(t, err, "failed AssignmentToDoc")

	// Concert the NavigaDoc back to an Assignment
	got, err := docconverter.DocToAssignment(document, opts)
	must(t, err, "failed DocToAssignment")
...
func planningItemConversionRoundtrip(t *testing.T, test TestData, opts *newsml.Options) {
	...
	document, err := docconverter.PlanningItemToDoc(&original, opts)
	must(t, err, "failed PlanningItemToDoc")

	// Concert the NavigaDoc back to a List
	got, err := docconverter.DocToPlanningItem(document, opts)
	must(t, err, "failed DocToPlanningItem")
...
func TestConceptItemCreatingExtraMetaEvent(t *testing.T) {
	...
	opts := newsml.DefaultOptions()
	document, err := docconverter.ConceptItemToDoc(concept, &opts)
	must(t, err, "failed ConceptItemToDoc")
...
func TestConceptNoMetadata(t *testing.T) {
	...
	// Read the xml into the ConceptItem
	err = internal.UnmarshalXML(want, &concept)
	must(t, err, "could not unmarshal file")
...
func TestXMLToDOCToXML(t *testing.T) {
	...
	for i := range tests {
		...
		t.Run(test.xml, func(t *testing.T) {
			...
			document, err := docconverter.XMLToDoc(string(want), &opts)
			must(t, err, "failed XMLToDoc")

			xmlDoc, err := docconverter.DocToItem(document, &opts)
			must(t, err, "failed DocToItem")
...
func TestDocDataTags(t *testing.T) {
	...
	testData, err := os.ReadFile(testDoc)
	must(t, err, "could not open file")
	...
	_, err = docconverter.DocToItem(&navigaDoc, &opts)
	if err == nil {
...
func TestIDFMissingNamespace(t *testing.T) {
	...
	xmlDoc, err := os.ReadFile(xmlFile)
	must(t, err, "could not open file")

	opts := newsml.DefaultOptions()
	document, err := docconverter.XMLToDoc(string(xmlDoc), &opts)
	must(t, err, "failed XMLToDoc")
	...
	item, err := docconverter.DocToItem(document, &opts)
	must(t, err, "failed DocToItem")
...
func TestEmptyBlocks(t *testing.T) {
	var tests = []struct {
		...
		{"testdata/empty-blocks-example.json", navigadoc.ErrEmptyBlock},
		{"testdata/empty-document-example.json", navigadoc.ErrEmptyDoc},
		{"testdata/text.json", nil},
	}

	for _, test := range tests {
		jsonData, err := os.ReadFile(test.testfile)
		if err != nil {
		...
		opts := newsml.DefaultOptions()
		_, err = docconverter.DocToNewsItem(&navigaDoc, &opts)
		if test.error == nil {
			if err != nil {
...
func TestReadLegacyDateValues(t *testing.T) {
	...
	b, err := os.ReadFile(path.Join("testdata", "article-minimum.xml"))
	if err != nil {
	...
	err = internal.NewXMLDecoder(bytes.NewReader(b)).Decode(&item)
	if err != nil {
	...
	_, err = docconverter.XMLToDoc(string(x), &opts)
	if err != nil {
	...
	for i := range tests {
		...
		if test.xml != "" {
			t.Run("WithoutOptions/"+test.xml, func(t *testing.T) {
				...
				xmlDocOriginal := internal.NewEtreeDoc()
				...
				generatedNewsItem, err := docconverter.DocToNewsItem(navigadoc, opts)
				must(t, err, "failed DocToNewsItem")
	...
	for i := range tests {
		...
		if test.xml != "" {
			t.Run("WithoutOptions/"+test.xml, func(t *testing.T) {
				...
				xmlDocGenerated := internal.NewEtreeDoc()
...
func TestConceptDuplicateProperties(t *testing.T) {
	...
	xmlDoc, err := os.ReadFile(testFile)
	must(t, err, "could not open file")

	conceptItem := &newsml.ConceptItem{}

	// Read the xml into the ConceptItem
	err = internal.UnmarshalXML(xmlDoc, conceptItem)
	must(t, err, "could not unmarshal file")
	...
	// Convert the ConceptItem to NavigaDoc format
	document, err := docconverter.ConceptItemToDoc(conceptItem, opts)
	must(t, err, "failed ConceptItemToDoc")
	...
	// Convert the NavigaDoc back to a ConceptItem
	conceptItem, err = docconverter.DocToConceptItem(document, opts)
	must(t, err, "failed DocToConceptItem")
...
func dateTestHelper(
	...
	navigadoc, err := docconverter.XMLToDoc(string(b), &opts)
	if shouldError {
		if err == nil {
...
func roundtripJSON(t *testing.T, test *TestData, diffOptions ...cmp.Option) {
	...
	t.Run(test.json, func(t *testing.T) {
		...
		defaultCase := func(document *doc.Document, opts *newsml.Options) (interface{}, error) {
			return docconverter.DocToNewsItem(document, opts)
		}

		opts := newsml.DefaultOptions()
		xmlItem, err := docconverter.DocToItemWithDefault(&want, &opts, defaultCase)
		must(t, err, "failed DocToItem")

		got, err := docconverter.ItemToDoc(xmlItem, &opts)
		must(t, err, "failed ItemToDoc")
...
func dumpStructComparison(t *testing.T, diff string) {
	...
	err := os.WriteFile(diffPath, []byte(diff), 0600)
...
func dumpDocs(t *testing.T, document *doc.Document, generatedDoc *doc.Document, originalXML []byte, generatedXML []byte, errors ...error) {
	...
	if len(originalXML) > 0 {
		...
		err := os.WriteFile(originalPath, originalXML, 0600)
	...
	if len(generatedXML) > 0 {
		...
		err := os.WriteFile(generatedPath, generatedXML, 0600)
