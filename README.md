...
The docconverter package has the following components

* Package github.com/navigacontentab/docconverter/newsml
      ...
      * contains functions for retrieving and merging conversion options
        ...
        * docconverter.MergeOptions(opts newsml.Options, custom *newsml.Options
  
* Package github.com/navigacontentab/docconverter/listpackage
      
      * contains support for converting between List/Package and NavigaDoc
    
* Package github.com/navigacontentab/docconverter/
      
      * converter.go contains these functions for converting from one format to the other
        * AssignmentToDoc(*newsml.Assignment, *newsml.Options) (*doc.Document, error)
        * DocToAssignment(*doc.Document, *newsml.Options) (*newsml.Assignment, error)
        * DocToConceptItem(*doc.Document, *newsml.Options)
        * DocToItem(*doc.Document, *newsml.Options) (interface{}, error)
        * DocToList(*doc.Document, *newsml.Options) (*listpackage.List, error)
        * DocToNewsItem(*doc.Document, *newsml.Options) (*newsml.NewsItem, error)
        * DocToPackage(*doc.Document, *newsml.Options) (*listpackage.Package, error)
        * DocToPlanningItem(*doc.Document, *newsml.Options) (*newsml.PlanningItem, error)
        * ConceptItemToDoc(*newsml.ConceptItem, *newsml.Options) (*doc.Document, error)
        * ItemToDoc(interface{}, *newsml.Options) (*doc.Document, error)
        * ListToDoc(*listpackage.List, *newsml.Options) (*doc.Document, error)
        * NewsItemToDoc(*newsml.NewsItem, *newsml.Options) (*doc.Document, error)
        * PackageToDoc(*listpackage.Package, *newsml.Options) (*doc.Document, error)
        * PlanningItemToDoc(*newsml.PlanningItem, *newsml.Options) (*doc.Document, error)
      * converter.go also contains these helper functions for XML
        * XMLToDoc(string, *newsml.Options) (*doc.Document, error)
...
The docconverter module has internal values which are used if the *newsml.Options parameter of the transformation functions is left nil. The default configuration may be obtained using newsml.DefaultOptions().

A custom configuration file may be supplied, however its values must be merged with the defaults in order to be used. This may be accomplished by using docconverter.MergeOptions as follows
...
For list types, i.e. x-im/ordered-list, docconverter use the package blue-monday to sanitize HTML in the list data. The _html-sanitize-options_ section allows for setting some bluemonday policy options in the configuration.
...
```go
    ...
    import (
	    ...
	    dc "github.com/navigacontentlab/docconverter"
	    "github.com/navigacontentlab/docconverter/newsml"
	    "github.com/navigacontentlab/docconverter/doc"
    ...
    newsitem, err := dc.DocToNewsItem(&navigaDoc, &opts)
	if err != nil {
		panic(err)
	}
...
```go
	...
	err = internal.UnmarshalXML(xmlIn, &newsItem)
	if err != nil {
		panic(err)
	}
    ...
    opts := newsml.DefaultOptions()
	navigaDoc, err := dc.NewsItemToDoc(&newsItem, &opts)
	if err != nil {
		panic(err)
	}
